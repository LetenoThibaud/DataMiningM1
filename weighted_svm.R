...
tune = tune(svm, as.factor(Bankrupt)  ~ ., data = train,
           kernel="radial",
           ranges=list(
              gamma = c(0.01, 0.1, 0.2, 0.5, 1, 2),
              class.weights=list(c("0"=1, "1"=20), c("0"=1, "1"=30), 
                                 c("0"=1, "1"=40), c("0"=1, "1"=50)),
             cost = c(0.01, 0.1, 0.2, 0.5, 1, 2)),
            tunecontrol = tune.control(sampling = "cross", cross = 5))

print(tune$best.parameters)
C_optimal = tune$best.parameters$cost
weight_optimal = tune$best.parameters$class.weights
gamma_optimal = tune$best.parameter$gamma

start = Sys.time()
model = svm(as.factor(Bankrupt) ~ ., data = train, kernel="radial",
            cost=C_optimal, gamma=gamma_optimal, class.weights = weight_optimal)
end = Sys.time()
