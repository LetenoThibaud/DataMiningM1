...
balanced_train_rose = ROSE(Bankrupt ~ ., data = train, seed = 1)
print(balanced_train_rose$data)
pca = prcomp(dataset[1:96])

#jpeg(file="ROSE_PCA_fig.jpeg")
autoplot(pca, data = dataset, colour = 'Bankrupt')
#dev.off()

# we set the cross validation with 5 folds
tune = tune(svm, as.factor(class)  ~ ., data = balanced_train_rose$data,
            kernel="linear",
            ranges=list(
...
start = Sys.time()
model = svm(as.factor(Bankrupt)  ~ ., data = balanced_train_rose$data,
            kernel="linear", cost=0.01, class.weights = c("0"=1, "1"=2))
end = Sys.time()
